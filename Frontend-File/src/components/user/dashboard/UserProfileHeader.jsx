  ...
  const handleAvatarChange = (event) => {
    ...
    reader.onloadend = async () => {
      ...
      if (updateProfilePicture) {
        const ok = await updateProfilePicture(base64String);
        if (!ok) {
        ...
        <div className="bg-white/10 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-6 sm:p-8 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex flex-col md:flex-row items-center text-center md:text-left gap-6">
            <div className="relative group shrink-0">
              ...
              <div className="relative w-24 h-24 sm:w-28 sm:h-28 bg-slate-800 rounded-full border-4 border-slate-950 flex items-center justify-center text-4xl sm:text-5xl font-black text-white shadow-xl overflow-hidden z-10 cursor-pointer" onClick={() => avatarInputRef.current?.click()} title="Ubah Foto Profil">
                {(avatarPreview || user?.profile_picture)
                  ? <img src={avatarPreview || user?.profile_picture} alt={user?.name} className="w-full h-full object-cover" />
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <button onClick={() => bannerInputRef.current?.click()} className="px-5 py-3 bg-white/10 hover:bg-white/20 border border-white/10 rounded-xl text-xs font-bold text-white transition-colors flex items-center justify-center gap-2 backdrop-blur-md"><Camera size={16} /> Ubah Banner Travel</button>
