...
const HeroSection = () => {
  ...
  const handleSearch = (e) => {
    ...
    navigate('/search-page', { 
      state: { startDate, endDate, selectedCity: selectedCity || "Stasiun Lempuyangan" } 
    });
  };
  ...
  return (
    <div className="relative pt-32 pb-32 px-4 overflow-hidden bg-slate-950">
      ...
      <div className="max-w-6xl mx-auto relative z-10 text-center animate-fade-in-up">
        <span className="bg-rose-500 text-white text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full mb-6 inline-block shadow-lg">#1 Rental Motor di Jogja & Solo</span>
        ...
        <form onSubmit={handleSearch} className="bg-white p-4 sm:p-5 rounded-3xl shadow-2xl max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-4 items-end relative z-20">
          <div className="md:col-span-1 text-left">
            <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest ml-1">Lokasi Tujuan</label>
            <div className="flex items-center gap-3 bg-slate-50 p-3.5 rounded-2xl border border-slate-200 focus-within:ring-2 focus-within:ring-rose-500 focus-within:border-rose-500">
              ...
              <select value={selectedCity || "Stasiun Lempuyangan"} onChange={(e) => setSelectedCity(e.target.value)} className="w-full bg-transparent outline-none font-bold text-slate-900 cursor-pointer text-sm appearance-none">
          ...
          <div className="text-left">
            ...
            <input type="date" value={startDate} min={today} onChange={(e) => setStartDate(e.target.value)} className="w-full bg-slate-50 p-3.5 rounded-2xl border border-slate-200 text-sm font-bold outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-500 transition-all cursor-pointer" />
          </div>
          <div className="text-left">
            <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest ml-1">Tanggal Kembali</label>
            <input type="date" value={endDate} min={startDate || today} onChange={(e) => setEndDate(e.target.value)} className="w-full bg-slate-50 p-3.5 rounded-2xl border border-slate-200 text-sm font-bold outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-500 transition-all cursor-pointer" />
          </div>
          <div>
