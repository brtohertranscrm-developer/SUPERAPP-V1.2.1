...
router.get('/employees', requirePermission('manning'), async (req, res) => {
  try {
    ...
    const rows = await dbAll(
      `SELECT e.* FROM employees e ${whereSql} ORDER BY e.is_active DESC, e.base_location ASC, e.name ASC`,
      params
    );
...
router.post('/employees', requirePermission('manning'), async (req, res) => {
  try {
    ...
    const r = await dbRun(
      `INSERT INTO employees (name, phone, email, base_location, role_tag, is_active)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [name, phone, email, base_location, role_tag, is_active]
    );

    const row = await dbGet(`SELECT * FROM employees WHERE id = ?`, [r.lastID]);
...
router.put('/employees/:id', requirePermission('manning'), async (req, res) => {
  try {
    ...
    await dbRun(
      `UPDATE employees
       SET name = ?, phone = ?, email = ?, base_location = ?, role_tag = ?, is_active = ?
       WHERE id = ?`,
      [name, phone, email, base_location, role_tag, is_active, id]
    );
